...
public class OAuth2PKCEAuthenticator: NSObject {
    ...
    public func authenticate(parameters: OAuth2PKCEParameters, completion: @escaping (Result<AccessTokenResponse, OAuth2PKCEAuthenticatorError>) -> Void) {
        ...
        // 4. redirects the user to the authorization server along with the code_challenge
        let authenticationSession = ASWebAuthenticationSession(
            url: URL(string: "\(parameters.authorizeUrl)?response_type=code&code_challenge=\(code_challenge)&code_challenge_method=S256&client_id=\(parameters.clientId)&redirect_uri=\(parameters.redirectUri)&scope=\(scope)".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!)!,
            callbackURLScheme: parameters.callbackURLScheme) { optionalUrl, optionalError in
                ...
                // 5. sends this code and the code_verifier (created in step 2) to the authorization server (token endpoint)
